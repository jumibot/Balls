package assets;


public final class ProjectAssets {

    public final AssetCatalog catalog;


    public ProjectAssets() {
        this.catalog = new AssetCatalog("src/resources/images/");
        this.catalog.register("cosmic_portal_1", "cosmic-portal-1.png", AssetType.COSMIC_PORTAL, AssetIntensity.HIGH);
        this.catalog.register("cracks_1", "cracks-1.png", AssetType.CRACKS, AssetIntensity.HIGH);
        this.catalog.register("cracks_2", "cracks-2.png", AssetType.CRACKS, AssetIntensity.HIGH);
        this.catalog.register("cracks_3", "cracks-3.png", AssetType.CRACKS, AssetIntensity.HIGH);
        this.catalog.register("cracks_4", "cracks-4.png", AssetType.CRACKS, AssetIntensity.HIGH);
        this.catalog.register("cracks_5", "cracks-5.png", AssetType.CRACKS, AssetIntensity.HIGH);
        this.catalog.register("halo_1", "halo-1.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_2", "halo-2.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_3", "halo-3.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_4", "halo-4.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_5", "halo-5.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_6", "halo-6.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_7", "halo-7.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_8", "halo-8.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_9", "halo-9.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_10", "halo-10.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_11", "halo-11.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_12", "halo-12.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("halo_13", "halo-13.png", AssetType.HALO, AssetIntensity.HIGH);
        this.catalog.register("light_1", "light-1.png", AssetType.LIGHT, AssetIntensity.HIGH);
        this.catalog.register("light_2", "light-21.png", AssetType.LIGHT, AssetIntensity.HIGH);
        this.catalog.register("shot_hole_1", "shot-hole-1.png", AssetType.SHOT_HOLE, AssetIntensity.HIGH);
        this.catalog.register("shot_hole_2", "shot-hole-2.png", AssetType.SHOT_HOLE, AssetIntensity.HIGH);
        this.catalog.register("shot_hole_3", "shot-hole-3.png", AssetType.SHOT_HOLE, AssetIntensity.HIGH);

        this.catalog.register("back_1", "bg-space-high-intensity-1.png", AssetType.BACKGROUND, AssetIntensity.HIGH);
        this.catalog.register("back_2", "bg-space-low-intensity-1.jpeg", AssetType.BACKGROUND, AssetIntensity.LOW);
        this.catalog.register("back_3", "bg-space-low-intensity-2.jpg", AssetType.BACKGROUND, AssetIntensity.LOW);
        this.catalog.register("back_4", "bg-space-medium-intensity-1.jpeg", AssetType.BACKGROUND, AssetIntensity.MEDIUM);
        this.catalog.register("back_5", "bg-space-medium-intensity-2.jpeg", AssetType.BACKGROUND, AssetIntensity.MEDIUM);

        this.catalog.register("black_hole_1", "black-hole-1.png", AssetType.BLACK_HOLE, AssetIntensity.HIGH);
        this.catalog.register("black_hole_2", "black-hole-2.png", AssetType.BLACK_HOLE, AssetIntensity.HIGH);
        this.catalog.register("moon_1", "moon-1.png", AssetType.MOON, AssetIntensity.HIGH);
        this.catalog.register("moon_2", "moon-2.png", AssetType.MOON, AssetIntensity.HIGH);
        this.catalog.register("planet_1", "planet-1.png", AssetType.PLANET, AssetIntensity.HIGH);
        this.catalog.register("planet_2", "planet-2.png", AssetType.PLANET, AssetIntensity.HIGH);
        this.catalog.register("planet_3", "planet-3.png", AssetType.PLANET, AssetIntensity.HIGH);
        this.catalog.register("planet_4", "planet-4.png", AssetType.PLANET, AssetIntensity.HIGH);
        this.catalog.register("planet_5", "planet-5.png", AssetType.PLANET, AssetIntensity.HIGH);
        this.catalog.register("planet_6", "planet-6.png", AssetType.PLANET, AssetIntensity.HIGH);
        this.catalog.register("sun_1", "sun-1.png", AssetType.SUN, AssetIntensity.HIGH);
        this.catalog.register("sun_2", "sun-2.png", AssetType.SUN, AssetIntensity.HIGH);
        this.catalog.register("sun_3", "sun-3.png", AssetType.SUN, AssetIntensity.HIGH);
        this.catalog.register("sun_4", "sun-4.png", AssetType.SUN, AssetIntensity.HIGH);

        this.catalog.register("asteroid_1", "asteroid-1-mini.png", AssetType.ASTEROID, AssetIntensity.HIGH);
        this.catalog.register("asteroid_2", "asteroid-2-mini.png", AssetType.ASTEROID, AssetIntensity.HIGH);
        this.catalog.register("asteroid_3", "asteroid-3-mini.png", AssetType.ASTEROID, AssetIntensity.HIGH);
        this.catalog.register("asteroid_4", "asteroid-4-mini.png", AssetType.ASTEROID, AssetIntensity.HIGH);
        this.catalog.register("asteroid_5", "asteroid-5-mini.png", AssetType.ASTEROID, AssetIntensity.HIGH);
        this.catalog.register("asteroid_6", "asteroid-6-mini.png", AssetType.ASTEROID, AssetIntensity.HIGH);
        this.catalog.register("meteor_1", "meteor-1.png", AssetType.METEOR, AssetIntensity.HIGH);
        this.catalog.register("meteor_2", "meteor-2.png", AssetType.METEOR, AssetIntensity.HIGH);
        this.catalog.register("meteor_3", "meteor-3.png", AssetType.METEOR, AssetIntensity.HIGH);
        this.catalog.register("meteor_4", "meteor-4.png", AssetType.METEOR, AssetIntensity.HIGH);
        this.catalog.register("meteor_4", "meteor-4.png", AssetType.METEOR, AssetIntensity.HIGH);

        this.catalog.register("bubbles_1", "bubbles-1-mini.png", AssetType.BUBBLES, AssetIntensity.HIGH);
        this.catalog.register("bubbles_2", "bubbles-2-mini.png", AssetType.BUBBLES, AssetIntensity.HIGH);
        this.catalog.register("bubbles_3", "bubbles-3-mini.png", AssetType.BUBBLES, AssetIntensity.HIGH);
        this.catalog.register("bubbles_4", "bubbles-4-mini.png", AssetType.BUBBLES, AssetIntensity.HIGH);
        this.catalog.register("galaxy_1", "galaxy-1.png", AssetType.GALAXY, AssetIntensity.HIGH);
        this.catalog.register("galaxy_2", "galaxy-2.png", AssetType.GALAXY, AssetIntensity.HIGH);
        this.catalog.register("galaxy_3", "galaxy-3.png", AssetType.GALAXY, AssetIntensity.HIGH);
        this.catalog.register("rainbow_1", "rainbow-1.png", AssetType.RAINBOW, AssetIntensity.HIGH);
        this.catalog.register("stars_1", "stars-1-mini.png", AssetType.STARS, AssetIntensity.HIGH);
        this.catalog.register("stars_2", "stars-2-mini.png", AssetType.STARS, AssetIntensity.HIGH);
        this.catalog.register("stars_3", "stars-3-mini.png", AssetType.STARS, AssetIntensity.HIGH);
        this.catalog.register("stars_4", "stars-4-mini.png", AssetType.STARS, AssetIntensity.HIGH);
        this.catalog.register("stars_5", "stars-5-mini.png", AssetType.STARS, AssetIntensity.HIGH);

        this.catalog.register("rocket_1", "rocket-1.png", AssetType.ROCKET, AssetIntensity.HIGH);
        this.catalog.register("rocket_2", "rocket-2.png", AssetType.ROCKET, AssetIntensity.HIGH);
        this.catalog.register("rocket_3", "rocket-3.png", AssetType.ROCKET, AssetIntensity.HIGH);
        this.catalog.register("rocket_4", "rocket-4.png", AssetType.ROCKET, AssetIntensity.HIGH);
        this.catalog.register("rocket_5", "rocket-5.png", AssetType.ROCKET, AssetIntensity.HIGH);
        this.catalog.register("rocket_6", "rocket-6.png", AssetType.ROCKET, AssetIntensity.HIGH);
        this.catalog.register("rocket_7", "rocket-7.png", AssetType.ROCKET, AssetIntensity.HIGH);
        this.catalog.register("spaceship_1", "spaceship-1.png", AssetType.SPACESHIP, AssetIntensity.HIGH);
        this.catalog.register("spaceship_2", "spaceship-2.png", AssetType.SPACESHIP, AssetIntensity.HIGH);
        this.catalog.register("spaceship_3", "spaceship-3.png", AssetType.SPACESHIP, AssetIntensity.HIGH);
        this.catalog.register("spaceship_4", "spaceship-4.png", AssetType.SPACESHIP, AssetIntensity.HIGH);
        this.catalog.register("spaceship_5", "spaceship-5.png", AssetType.SPACESHIP, AssetIntensity.HIGH);
        this.catalog.register("spaceship_6", "spaceship-6.png", AssetType.SPACESHIP, AssetIntensity.HIGH);
        this.catalog.register("spaceship_7", "spaceship-7.png", AssetType.SPACESHIP, AssetIntensity.HIGH);
        this.catalog.register("lab_1", "lab-1.png", AssetType.LAB, AssetIntensity.HIGH);
        this.catalog.register("lab_2", "lab-2.png", AssetType.LAB, AssetIntensity.HIGH);
        this.catalog.register("lab_3", "lab-3.png", AssetType.LAB, AssetIntensity.HIGH);
        this.catalog.register("lab_4", "lab-4.png", AssetType.LAB, AssetIntensity.HIGH);

        this.catalog.register("signs_1", "ui-signs-1.png", AssetType.UI_SIGN, AssetIntensity.HIGH);

        this.catalog.register("bullet_1", "bullet-1.png", AssetType.BULLET, AssetIntensity.LOW);
        // this.catalog.register("bomb_1", "grenade-1.png", AssetType.MINE, AssetIntensity.MEDIUM);
        this.catalog.register("bomb_2", "grenade-2.png", AssetType.MINE, AssetIntensity.MEDIUM);
        this.catalog.register("misil_1", "misil-1-mini.png", AssetType.MISSILE, AssetIntensity.HIGH);
        this.catalog.register("misil_2", "misil-2-mini.png", AssetType.MISSILE, AssetIntensity.HIGH);
        this.catalog.register("misil_3", "misil-3-mini.png", AssetType.MISSILE, AssetIntensity.HIGH);
        this.catalog.register("misil_4", "misil-4.png", AssetType.MISSILE, AssetIntensity.HIGH);
        this.catalog.register("misil_5", "misil-5-mini.png", AssetType.MISSILE, AssetIntensity.HIGH);
    }
}
