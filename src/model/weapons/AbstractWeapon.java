 ...
 *
 * This class provides: • Immutable identification (weapon id) • A static
 * configuration object (WeaponDto) • A thread-safe monotonic firing-request
 * mechanism • A unified API for discrete-tick weapon updates
 *
 * CONCURRENCY MODEL: ------------------ - The method registerFireRequest() may
 * be invoked from *any thread* (typically from the Controller or Model in
 * response to player input). It stores a timestamp (System.nanoTime) in an
 * AtomicLong.
 *
 ...
 *
 * FIRING REQUEST MODEL: --------------------- - lastFireRequest holds the
 * timestamp of the *latest* firing request. - lastHandledRequest holds the
 * timestamp of the most recent request already processed (shot produced or
 * consciously ignored).
 *
 ...
 *
 * Calling hasNewRequest() consumes the request by advancing lastHandledRequest
 * to lastFireRequest.
 *
 ...
 *
 * DESIGN PHILOSOPHY: ------------------- - Weapons do not run code outside
 * update(): deterministic, thread-safe, and easy to reason about in a heavily
 * multi-threaded simulation.
 *
 * - Firing intent is monotonic and edge-triggered. The system never stores
 * multiple pending shots; weapons decide immediately whether to fire or ignore
 * a request.
 *
 ...
 * weapon type, follow these principles:
 *
